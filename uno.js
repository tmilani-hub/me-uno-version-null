		...
		for (let y = 0; y < howManyCard; y++) {
			...
			if (random <= 3) {
				...
				if (random <= 3) {
					//  num
					isadd != null
						? isadd.push(createObject(selectCard("red", "num")))
						: cardsOfOnePlayer.cardOF.push(
								createObject(selectCard("red", "num")),
							);
				} else {
					//  spe
					isadd != null
						? isadd.push(createObject(selectCard("red", "spe")))
						: cardsOfOnePlayer.cardOF.push(
								createObject(selectCard("red", "spe")),
							);
				}
				...
				if (random <= 3) {
					//  num
					isadd != null
						? isadd.push(createObject(selectCard("blue", "num")))
						: cardsOfOnePlayer.cardOF.push(
								createObject(selectCard("blue", "num")),
							);
				} else {
					//  spe
					isadd != null
						? isadd.push(createObject(selectCard("blue", "spe")))
						: cardsOfOnePlayer.cardOF.push(
								createObject(selectCard("blue", "spe")),
							);
				}
				...
				if (random <= 3) {
					//  num
					isadd != null
						? isadd.push(createObject(selectCard("yellow", "num")))
						: cardsOfOnePlayer.cardOF.push(
								createObject(selectCard("yellow", "num")),
							);
				} else {
					//  spe
					isadd != null
						? isadd.push(createObject(selectCard("yellow", "spe")))
						: cardsOfOnePlayer.cardOF.push(
								createObject(selectCard("yellow", "spe")),
							);
				}
				...
				if (random <= 3) {
					//  num
					isadd != null
						? isadd.push(createObject(selectCard("green", "num")))
						: cardsOfOnePlayer.cardOF.push(
								createObject(selectCard("green", "num")),
							);
				} else {
					//  spe
					isadd != null
						? isadd.push(createObject(selectCard("green", "spe")))
						: cardsOfOnePlayer.cardOF.push(
								createObject(selectCard("green", "spe")),
							);
				}
			} else {
				random = Math.floor(Math.random() * 2);
				isadd != null
					? isadd.push(createObject(Allcard.black[random]))
					: cardsOfOnePlayer.cardOF.push(createObject(Allcard.black[random]));
			}
		}
...
function giveMeColor() {
	...
	IcolorChoose.addEventListener("input", () => {
		if (
			IcolorChoose.value == "rouge" ||
			IcolorChoose.value == "jaune" ||
			IcolorChoose.value == "vert" ||
			IcolorChoose.value == "bleu"
		) {
	...
	let tab = [
		document.querySelector("#otherPlayer"),
		document.querySelector("#gameActive"),
		document.querySelector("#piocheEtDefause"),
		document.querySelector("#tourDuJoueur"),
	];
	for (let i = 0; i < tab.length; i++) {
		if (tab[i] != undefined) {
					...
					if (card.color == "black") {
						...
						if (card.id.includes("Pastrn")) {
							...
							for (let i = 0; i < AllPlayer[auTourDe].cardOF.length; i++) {
								if (AllPlayer[auTourDe].cardOF[i].color == "black") {
									console.log("black card");
									continue;
								}
								if (AllPlayer[auTourDe].cardOF[i].id.includes("Addtwo")) {
									horlosens
										? auTourDe - 1 > -1
											? (auTourDe = auTourDe - 1)
											: (auTourDe = AllPlayer.length - 1)
...
function verrifyCard(carte1, carte2, CAsk) {
	...
	if (carte1.spe) {
		if (carte1.color == "black") {
			...
			if (
				(carte1.id.includes("Addtwo") && carte2.id.includes("Addtwo")) ||
				(carte1.id.includes("Pastrn") && carte2.id.includes("Pastrn")) ||
				(carte1.id.includes("CgeSns") && carte2.id.includes("CgeSns"))
			) {
...
function actulize() {
	...
	for (let i = 0; i < $infoDeJ_num.length; i++) {
		if (AllPlayer[i].cardOF.length == 1) {
			$infoDeJ_num[i].textContent =
				"Le joueur " +
				AllPlayer[i].name +
				" a " +
				AllPlayer[i].cardOF.length +
				" carte .";
		} else {
			$infoDeJ_num[i].textContent =
				"Le joueur " +
	...
	pioche.addEventListener("click", () => {
		AllPlayer[auTourDe].cardOF = addorset(
			null,
			false,
			1,
			AllPlayer[auTourDe].cardOF,
		);
		tourSuivant();
		vueCardOf(AllPlayer[auTourDe]);
		boom(AllPlayer[auTourDe].name);
		boom(
			null,
			"tu a piocher la carte " +
				AllPlayer[
					horlosens
						? auTourDe - 1 == -1
							? AllPlayer.length - 1
							: auTourDe - 1
						: auTourDe + 1 > AllPlayer.length - 1
							? 0
							: auTourDe + 1
				].cardOF[
					AllPlayer[
